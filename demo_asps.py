...
# ============================
# 5. GLOBAL TOKENIZER & MODEL - LAZY LOADING
# ============================

from transformers import AutoTokenizer, AutoModelForCausalLM

# Global variables to be loaded lazily
tokenizer = None
rag_model = None

def load_models():
    """Load tokenizer and model lazily to avoid blocking startup"""
    global tokenizer, rag_model
...
# ============================
# 6. GLOBAL EMBEDDING MODEL - LAZY LOADING
# ============================

from sentence_transformers import SentenceTransformer
import logging

# Global embedding model to be loaded lazily
embed_model = None

def load_embedding_model():
    """Load embedding model lazily to avoid blocking startup"""
    global embed_model
    ...
    if embed_model is None:
        ...
        try:
            ...
            print(f"✅ Loaded embedding model '{EMBEDDING_MODEL_NAME}' on device {DEVICE}")
...
def retrieve_context(query, k=3, initial_k=10, intent=None):
    ...
    """
    # Load embedding model when needed
...
async def startup_event():
    ...
    print("ℹ️  Knowledge bases will be loaded on first query to prevent startup hanging")
    # Don't call initialize_asps_system() here - let it load lazily on first query
...
async def query_asps_rag(request: QueryRequest):
    try:
        # Lazy load knowledge bases on first query
        if not (FAISS_INDEXES.get("clinical") or FAISS_INDEXES.get("navigation")):
            print("🔄 First query detected - loading knowledge bases...")
            if not load_github_knowledge_bases():
                raise HTTPException(status_code=503, detail="Failed to load knowledge bases. Please try again.")
...
if __name__ == "__main__":
    ...
    print("ℹ️  Knowledge bases and models will load on first query to prevent startup hanging")
...
if __name__ == "__main__":
    ...
    # Initialize ASPS system (this loads knowledge bases but NOT models yet)
    print("🚀 Step 1: Initializing knowledge bases...")
    initialize_asps_system()
    
    # Start FastAPI server
    print("\n🌐 Step 2: Starting ASPS Demo API server...")
    ...
    print("ℹ️  Note: Models will be loaded lazily when first query is made")
...
def main():
    ...
    print("🏥 Health check at: http://213.173.110.81:19524/health")
    print("🔗 RunPod External Access: Connect via TCP port 213.173.110.81:19524")
    print("ℹ️  Note: Knowledge bases and models will load lazily on first query")
    print("\n✅ SYSTEM READY FOR CLINICAL/NAVIGATION DIFFERENTIATION!")
    uvicorn.run(app, host="0.0.0.0", port=19524)
